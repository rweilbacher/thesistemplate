...
The preemptive means that if a process with a higher priority than the currently running process becomes ready to be executed, the currently running process gets interrupted.

\paragraph{Time-sliced round-robin scheduling}


* The scheduling for the microkernel processes is preemptive priority based and time-sliced round robin 
    -> Explain them a bit and add image
* But there is no unified scheduling behavior as that can heavily depend on the requirements of the system
* However, they all need to be deterministic to allow for WCET analysis
* Hypervisors allow for multiple scheduling plans to be defined that can be switched between at runtime by an authorized partition
    -> Some possible applications for this would be special scheduling during the boot process or to put the device into a maintenance mode after an unexpected error has occurred.
