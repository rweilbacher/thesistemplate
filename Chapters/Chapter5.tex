...
* How is the device access precisely configured?
* What is the scheduling algorithm and plan?
    -> Can I justify round-robin?
    -> A simplified priority based preemptive is probably good. The critical part would often be dormant and this way the UI only gets starved of resources during measurement. That can be addressed by GUI design
* What are the details of the partition communication?
* Health monitoring?
    -> Application B verifies through log stream that C is not running into issues.

-> Why a decision has been made should always be clear. If there is no clear reason just say that it was not clear-cut and the other decision would have probably be fine too.
